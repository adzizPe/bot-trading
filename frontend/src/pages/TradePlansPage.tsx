...
export function TradePlansPage() {
  ...
  const canCreate = usePermission('trade-plan:create')
  const canManagePaper = usePermission('paper:trade')
  const canExecuteDemo = usePermission('demo:execute')
  ...
  const requestDemo = (plan: TradePlan) => setDemoPlan(plan)

  return <div className="page-stack">
    <PageHeader title="Trade plans" description="Risk plan dapat dipakai untuk paper trading atau eksekusi manual pada akun MT5 demo yang terverifikasi." actions={canCreate && <button className="button button-primary" disabled={!signal.data || signal.data.direction === 'HOLD' || create.isPending} onClick={() => create.mutate()}><Calculator size={16} />{create.isPending ? 'Calculating…' : 'Buat dari latest signal'}</button>} />
    ...
    {plans.data?.length ? <DataTable rows={plans.data} rowKey={(item) => item.trade_plan_id} columns={[
      ...
      {selected && <div className="dialog-backdrop"><div className="dialog dialog-wide" role="dialog" aria-modal="true" aria-label="Trade plan detail"><TradePlanCard plan={selected} action={selected.status === 'APPROVED' && (canManagePaper || canExecuteDemo) && <div className="engine-actions">{canManagePaper && <button className="button button-ghost" onClick={() => setPaperPlan(selected)}>Open paper position</button>}{canExecuteDemo && <button className="button button-danger" onClick={() => requestDemo(selected)}>Execute Demo</button>}</div>} /><Panel title="Calculation details"><pre className="json-detail">{JSON.stringify(selected.calculation_details, null, 2)}</pre></Panel><button className="button button-ghost" onClick={() => setSelected(null)}>Tutup</button></div></div>}
