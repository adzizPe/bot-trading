...
export function PaperPage() {
  ...
  const canControl = usePermission('paper:control')
  const canManagePositions = usePermission('paper:trade')
  ...
  return <div className="page-stack">
    <PageHeader title="Paper trading" description="Lifecycle posisi simulasi dengan quote MT5 demo read-only." actions={canControl && <div className="engine-actions">
    ...
    <Panel title="Open positions" subtitle="Harga dan floating PnL diperbarui oleh paper scheduler.">
      {positions.data?.filter((item) => item.status === 'OPEN').length ? <DataTable rows={positions.data.filter((item) => item.status === 'OPEN')} rowKey={(item) => item.position_id} columns={[
        ...
        { key: 'action', header: '', cell: (item) => canManagePositions ? <button className="button button-ghost" onClick={() => { setPosition(item); setConfirm('close') }}>Close</button> : null },
