...
export function RiskPage() {
  ...
  const canWrite = usePermission('risk:settings:update')
  ...
  return <div className="page-stack">
    ...
    <Panel title="Risk settings" subtitle="Perubahan penting memerlukan konfirmasi.">
      {settings.isLoading ? <LoadingSkeleton rows={6} /> : settings.error ? <ErrorAlert message={sanitizeMessage(settings.error)} retry={() => settings.refetch()} /> : <form className="form-grid" onSubmit={submit} noValidate>
        ...
        {canWrite && <div className="form-actions"><button className="button button-primary" disabled={update.isPending}><Save size={16} />Simpan pengaturan</button></div>}
      </form>}
    </Panel>
    <ConfirmDialog open={canWrite && confirm} title="Ubah risk settings?" description="Batas risiko baru akan digunakan pada trade plan berikutnya. Pastikan nilainya sudah ditinjau." busy={update.isPending} onCancel={() => setConfirm(false)} onConfirm={() => update.mutate()} />
  </div>
}
