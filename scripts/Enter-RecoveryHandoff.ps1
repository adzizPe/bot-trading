...
param(
    ...
    [string]$EvidenceId = 'recovery-handoff-plan', [string]$EnvironmentSource = '',
...
Invoke-OperationsCli -Operation 'recovery-handoff' -Root $Root -ProcessManager $ProcessManager -TimeoutSeconds $TimeoutSeconds -EvidenceId $EvidenceId -EnvironmentSource $EnvironmentSource -PythonPath $PythonPath -OperationArguments $operationArguments -Execute:$Execute -WhatIfMode ([bool]$WhatIfPreference)
